...
<div class="content-wrapper">
  <div class="content-heading">
   <span><em class="fa fa-group"></em> Leads</span>
   <div class="pull-right"><a href="{{ route('leads.create') }}" class="btn btn-sm btn-info"> Add lead </a></div>
 </div>
    ...
    <div class="table-responsive">
     ...
     <tbody>
      ...
      <tr>
        ...
        <td>{{ $row->phone }}</td>
        <td>{{ $row->email }}</td>
        <td>@if($row->social_id){{ $row->social->name }}@endif</td>
        <td>
        ...
        <td>
          <select class="form-control select2-4" name="user_id" onchange="setUser(this,{{ $row->id }});">
           <option value="">Select</option>
           @foreach ($users as $user)
             <option value="{{ $user->id }}" {{ ($user->id==$row->user_id)? 'selected' : '' }}>{{ $user->first_name }} {{ $user->last_name }}</option>
           @endforeach
          </select>
        </td>
        <td>
          <a href="{{ route('leads.edit',['id'=>base64_encode($row->id)]) }}" class="btn btn-info btn-xs">Edit</a>
          <a href="{{ route('leads.show',['id'=>base64_encode($row->id)]) }}" class="btn btn-info btn-xs">Next</a>
