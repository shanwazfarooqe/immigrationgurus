  ...
  <div class="panel-body">
    ...
    <div class="tab-content" style="background-color:#FFFFFF">
      <div id="home" role="tabpanel" class="tab-pane active">
        <form class="form-horizontal" action="{{ route('orgnotes.store') }}" method="post">
        @csrf
      ...
      <div id="profile" role="tabpanel" class="tab-pane">
        <form class="form-horizontal" action="{{ route('orgactivities.store') }}" method="post">
        @csrf
          <input type="hidden" name="organization_id" value="{{ $organization->id }}">
          <input type="hidden" name="user_id" value="{{ auth()->id() }}">
      ...
      <div id="messages" role="tabpanel" class="tab-pane">
        <form class="form-horizontal" action="{{ route('orgtasks.store') }}" method="post">
        @csrf
          <input type="hidden" name="organization_id" value="{{ $organization->id }}">
          <input type="hidden" name="user_id" value="{{ auth()->id() }}">
          <div class="row" style="margin-top:15px;">
         ...
         <table class="table table-striped table-hover">
          ...
          @foreach($email_logs as $row)
            <tr>
             ...
             <td><a href="{{ route('orgemaillogs.show',['id'=>base64_encode($row->id)]) }}">{{ $row->subject }}</a></td>
 ...
 <div class="timeline-panel">
   <div class="popover right">
     <div class="popover-title" style="color:#182429;background-color:#FFF; border-bottom:#CECECE .5px solid; height:40px;">
      <span class="pull-right"><a href="#" data-toggle="modal" data-target="#noteModal" data-action="{{ route('orgnotes.update',['id'=>$row->id]) }}" data-content="{{ $row->content }}">Edit</a>&nbsp;&nbsp;<a  href="javascript:void(0)" onclick="deleteNote({{ $row->id }})">Delete</a></span>
      <form action="{{ route('orgnotes.destroy',['id'=>$row->id]) }}" method="post" id="note-delete{{ $row->id }}" style="display: none;">
      @csrf
      @method('delete')
      <input type="hidden" name="id" value="{{ $row->id }}">
 ...
 <div class="timeline-panel">
   <div class="popover right">
     <div class="popover-title" style="color:#182429;background-color:#FFF; border-bottom:#CECECE .5px solid; height:40px;">
       ...
       <span class="pull-right"><a href="#" data-toggle="modal" data-target="#logModal" data-action="{{ route('orgactivities.update',['id'=>$row->id]) }}" data-content="{{ $row->content }}" data-log_a_call="{{ $row->log_a_call }}" data-out_come="{{ $row->out_come }}" data-datetime="{{ $row->datetime }}">Edit</a>&nbsp;&nbsp;<a href="javascript:void(0)" onclick="deleteActivity({{ $row->id }})">Delete</a></span>
       <form action="{{ route('orgactivities.destroy',['id'=>$row->id]) }}" method="post" id="activity-delete{{ $row->id }}" style="display: none;">
       @csrf
       @method('delete')
       <input type="hidden" name="id" value="{{ $row->id }}">
       <button class="btn btn-danger" type="submit">Delete</button>
 ...
 <div class="timeline-panel">
   <div class="popover right">
     <div class="popover-title" style="color:#182429;background-color:#FFF; border-bottom:#CECECE .5px solid; height:40px;">
      ...
      <span class="pull-right"><a href="#" data-toggle="modal" data-target="#taskModal" data-action="{{ route('orgtasks.update',['id'=>$row->id]) }}" data-content="{{ $row->content }}" data-subject="{{ $row->subject }}" data-datetime="{{ $row->datetime }}">Edit</a>&nbsp;&nbsp;<a href="javascript:void(0)" onclick="deleteTask({{ $row->id }})">Delete</a></span>
      <form action="{{ route('orgtasks.destroy',['id'=>$row->id]) }}" method="post" id="task-delete{{ $row->id }}" style="display: none;">
      @csrf
      @method('delete')
      <input type="hidden" name="id" value="{{ $row->id }}">
      <button class="btn btn-danger" type="submit">Delete</button>
...
@section('custom_js')
  ...
  <div class="modal fade" id="email_modal" role="dialog">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <form method="post" action="{{ route('orgemaillogs.store') }}" class="form-horizontal" id="email-modal-edit">
        @csrf
        <input type="hidden" name="organization_id" value="{{ $organization->id }}">
        <input type="hidden" name="template_id">
          ...
          <div class="modal-body">
           <div class="form-group form_m0 loop_mail">
            ...
            <div class="col-sm-9">
              <input type="text" placeholder="To" class="form-control b0" name="to" value="{{ $organization->email }}" readonly="">
            </div>
            <div class="col-sm-2">
        ...
        <div class="form-group ">
         <div class="col-sm-12">
           <input type="text" placeholder="Header" value="Dear {{ $organization->first_name }} {{ $organization->last_name }}," class="form-control b0" readonly="">
         </div>
       </div>
  ...
  <div class="modal fade" id="email_modal_add" role="dialog">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <form method="post" action="{{ route('orgemaillogs.store') }}" class="form-horizontal" id="email-modal-add">
        @csrf
        <input type="hidden" name="organization_id" value="{{ $organization->id }}">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal">&times;</button>
          ...
          <div class="modal-body">
           <div class="form-group form_m0 loop_mail">
            ...
            <div class="col-sm-9">
              <input type="text" placeholder="To" value="{{ $organization->email }}" class="form-control b0" name="to" readonly="">
            </div>
            <div class="col-sm-2">
        ...
        <div class="form-group ">
         <div class="col-sm-12">
          <input type="text" placeholder="" value="Dear {{ $organization->first_name }} {{ $organization->last_name }}," class="form-control b0" readonly="">
        </div>
      </div>
       ...
       .done(function(data) {
         ...
         else
         {
           sweetAlert({
            ...
            function(isConfirm){
               location.href="{{ route('organizations.show',['id'=>base64_encode($organization->id)]) }}";
             });
         }
       });
